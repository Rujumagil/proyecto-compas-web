(() => {
  if (window.__COMPAS_BRAND_FIX_V22__) return;
  window.__COMPAS_BRAND_FIX_V22__ = true;

  const SPRITE = 'compas-logos-oficiales.png?v=22';
  const TRANSPARENT = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGNgYGBgAAAABQABpfZFQAAAAABJRU5ErkJggg==';
  const POSITIONS = { evolution:'0% 0%', one:'0% 25%', academia:'0% 50%', creators:'0% 75%', ia:'0% 100%' };

  const css=document.createElement('link');
  css.rel='stylesheet'; css.href='brand-visibility-v2.css?v=22'; css.dataset.compasBrandVisibility='true';
  ...
  document.querySelectorAll('.brand').forEach(brand=>{
    let img=brand.querySelector('img'); if(!img){ img=document.createElement('img'); brand.prepend(img); }
    img.alt=''; img.setAttribute('aria-hidden','true'); applyLogo(img,'evolution');
  });

  const isIaPage=/compas-ia\.html$/i.test(location.pathname)||document.title.toLowerCase().includes('compás ia');
  if(isIaPage){ const hero=document.querySelector('.page-hero .container'); if(hero&&!hero.querySelector('.page-icon')){ const img=document.createElement('img'); img.alt='Compás IA'; img.className='page-icon'; const kicker=hero.querySelector('.kicker'); kicker?hero.insertBefore(img,kicker):hero.prepend(img); applyLogo(img,'ia'); } }

  const numbered=['.manifesto-number','.case-number','.index','.orbit-node small','.feature-panel > span:first-child'];
  document.querySelectorAll(numbered.join(',')).forEach(el=>{ const text=(el.textContent||'').trim(); if(/^0?\d{1,2}$/.test(text)) return el.remove(); const clean=text.replace(/^0?\d{1,2}\s*[·•—–-]\s*/u,'').trim(); if(clean!==text) clean?el.textContent=clean:el.remove(); });
})();